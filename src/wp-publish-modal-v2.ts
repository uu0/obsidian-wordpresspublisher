...
import { getApiLimitations, getApiRecommendation } from './api-capability';
...
export class WpPublishModalV2 extends AbstractModal implements PublishModalContext {
  ...
  private contentPreviewSection: ContentPreviewSection | null = null; // 预览区内容段实例，供 footer 调用 enterEditMode()
   ...
   * 加载远程特色图片（别名方法）
   */
  async loadRemoteFeaturedImage(postId: string | number, _params: WordPressPostParams): Promise<void> {
  ...
  private renderV3PreviewArea(container: HTMLElement, params: WordPressPostParams): void {
    ...
    // 2. 文章内容段（内嵌摘要 + 标签）—— 持有实例以暴露 enterEditMode()
  ...
  private renderV3Footer(container: HTMLElement, params: WordPressPostParams): void {
    ...
    editBtn.onclick = () => {
      // 通过 Content section 实例的公开方法进入编辑模式（不再依赖 DOM 挂载）
      this.contentPreviewSection?.enterEditMode();
    };
  ...
  private async generateAImage(params: WordPressPostParams): Promise<void> {
    ...
    try {
      ...
      // 检测文章语言并生成图片描述提示词
      const contentToDetect = (params as any).content || this.articleContent;
  ...
  doPublish(params: WordPressPostParams, _btn?: HTMLButtonElement): void {
    // 防止重复点击
