...
**新增功能**
- 分类选择器：全新的标签式分类选择器 UI，显示当前分类、支持删除、通过+号下拉添加
- 发布为新文章：新增选项允许将同一内容发布为不同的文章
- 特色图片同步：添加特色图片同步和冲突检测功能
- 远程图片加载：添加超时重试功能和加载状态提示

**问题修复**
- 修复打开发布模态框时特色图片不显示的问题
- 修复重新发布时 featurePicture/featuredImageId 被清空的问题
- 修复冲突解决后分类显示为"未分类"的问题
- 修复预览标签删除后位置变化的问题
